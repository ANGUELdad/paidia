...
Mobile-first PWA, δίγλωσσο (DE/EL). Ένα αρχείο, χωρίς build.
Τα λειτουργικά δεδομένα (πρόγραμμα, αποθήκη, λίστες, events, προφίλ, PIN, Face ID)
συγχρονίζονται στον **server** μέσω `/api/ops` + `db.py`.

**Vercel:** χωρίς `DATABASE_URL` (Postgres / Neon) τα δεδομένα χάνονται σε κάθε deploy.
Τοπικά χρησιμοποιείται αυτόματα το SQLite `.paidia.db`. Στο Προφίλ εμφανίζεται πράσινο
banner όταν ο durable storage είναι ενεργός.
...
`openai/gpt-oss-120b`. Το κλειδί διαβάζεται μόνο από το `server.py` και δεν
αποστέλλεται ποτέ στον browser.

Το στρογγυλό κουμπί **?** / Chat ανοίγει **Zo-Ai** (και Tutorial / Team Talk). Η Zo-Ai
στέλνει το κείμενο της ερώτησης και γενικό context της οθόνης· οι αλλαγές σε Lager /
...
πτυσσόμενες κατηγορίες. Η προβολή «όλα τα σπίτια» χρησιμοποιεί τα ίδια συμπαγή
cards και εμφανίζει τις ποσότητες Kalyvia, Limenaria, Julian groß, Valeria και Lea
δίπλα-δίπλα.
...
## Υποχρεωτική ξενάγηση σε κάθε σύνδεση

Κάθε profile ολοκληρώνει μία role-specific ξενάγηση μετά από κάθε νέα επιτυχημένη
σύνδεση με PIN ή passkey. Ένα απλό refresh που επαναφέρει το ίδιο ενεργό session δεν
ξεκινά ξανά την ξενάγηση, εκτός αν το profile δεν έχει ολοκληρώσει ακόμη την τρέχουσα version.
...
κατηγορίες και αναζήτηση. Το cart έχει γρήγορη προσθήκη, εμφανή `+ / −` ποσότητας και
ένα σαφές sticky κουμπί έναρξης αγοράς. Διατηρεί ιστορικό ανά ολοκληρωμένο Einkauf και σπίτι, με
ξεχωριστές ενότητες για όσα αγοράστηκαν και όσα δεν αγοράστηκαν. Κάθε snapshot κρατά
Παρασκευή, ώρα ολοκλήρωσης και υπεύθυνο, ώστε μεταγενέστερες αλλαγές ή carry-over να μην
αλλοιώνουν το ιστορικό. Στον Einkaufsmodus τα **αγοράστηκε** και **δεν ήταν διαθέσιμο**
...
Κάθε αγορασμένο προϊόν—ακόμη και νέο όνομα από OCR—δημιουργείται/συνδέεται με inventory
product και αυξάνει αυτόματα το απόθεμα μόνο του επιλεγμένου σπιτιού. Τα νέα προϊόντα
εμφανίζονται στην κατηγορία **Weitere Produkte / Άλλα προϊόντα**.
