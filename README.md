...
```bash
cd ~/paidia
export OPENAI_API_KEY="..."       # required only for AI text/photo extraction
python3 server.py
```
...
Χωρίς `OPENAI_API_KEY`, η επικόλληση κειμένου συνεχίζει με τον τοπικό parser και εμφανίζει
σαφή προειδοποίηση. Η φωτογραφία δεν παράγει ψεύτικο αποτέλεσμα: ζητά να ρυθμιστεί το AI.
Το προαιρετικό `OPENAI_OCR_MODEL` αλλάζει το μοντέλο (default: `gpt-5.6-sol`). Το κλειδί
διαβάζεται μόνο από το `server.py` και δεν αποστέλλεται ποτέ στον browser.

## AI εισαγωγή λίστας (§58–§59)

Το **🪄 Εισαγωγή λίστας** δέχεται επικολλημένο κείμενο, εικόνα ή screenshot. Το AI
